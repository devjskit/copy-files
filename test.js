...
test("missing file operands", async (t) => {
  await t.throwsAsync(execa("./index.js"), { message: /`source` and `destination` required/ });
});

test("source file does not exist", async (t) => {
  await t.throwsAsync(execa("./index.js", [path.join(t.context.tmp, "nonexistentfile"), t.context.tmp]), { message: /nonexistentfile/ });
});
...
test("cwd", async (t) => {
  ...
  await execa("./index.js", ["hello.js", "dest", "--cwd", path.join(t.context.tmp, "cwd")]);
...
test("path structure", async (t) => {
  ...
  await execa("./index.js", [path.join(t.context.tmp, "**"), path.join(t.context.tmp, "out")]);

  t.is(read(t.context.tmp, "cwd/hello.js"), read(t.context.tmp, "out/cwd/hello.js"));
});
...
test("rename filenames but not filepaths", async (t) => {
  ...
  await execa("./index.js", [path.join(t.context.tmp, "hello.js"), path.join(t.context.tmp, "dest"), "--rename=hi.js"]);

  t.is(read(t.context.tmp, "hello.js"), read(t.context.tmp, "dest/hi.js"));

  await execa("./index.js", [path.join(t.context.tmp, "hello.js"), path.join(t.context.tmp, "dest"), "--rename=hi-{{basename}}-1"]);
...
test("overwrite files by default", async (t) => {
  ...
  await execa("./index.js", [path.join(t.context.tmp, "hello.js"), path.join(t.context.tmp, "dest")]);
...
test("do not copy files in the negated glob patterns", async (t) => {
  ...
  await execa("./index.js", ["src/*.*", "!src/*.jsx", "!src/*.es2015", path.join(t.context.tmp, "dest"), "--cwd", t.context.tmp]);
...
test("flatten directory tree", async (t) => {
  ...
  await execa("./index.js", ["**/*.js", "destination/subdir", "--cwd", t.context.tmp, "--flat"]);

  t.is(read(t.context.tmp, "foo.js"), read(t.context.tmp, "destination/subdir/foo.js"));
  t.is(read(t.context.tmp, "source/bar.js"), read(t.context.tmp, "destination/subdir/bar.js"));
